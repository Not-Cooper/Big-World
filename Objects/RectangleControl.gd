...
@export var horizontal_mod = 1.0
@export var vertical_mod = 1.0
@export var interpolation = .2
@export var max_scale = Vector2(3.0, 3.0)
		...
		if (current_scale < max_scale):
			if scale_constraint > 0:
				if (current_scale < Vector2(scale_constraint, scale_constraint)):
					...
					collider.scale = lerp(current_scale, Vector2(current_scale.x + scale_amount/vertical_mod, current_scale.y + scale_amount/horizontal_mod), interpolation)
			else:
				if scale_timer.is_stopped():
					scale_timer.start()
					start_scale = collider.scale
